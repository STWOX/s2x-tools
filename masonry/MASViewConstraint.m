...
- (NSMutableSet *)mas_installedConstraints {
  ...
  if (!constraints) {
    ...
    objc_setAssociatedObject(self, &kInstalledConstraintsKey, constraints,
...
- (id)copyWithZone:(NSZone __unused *)zone {
  MASViewConstraint *constraint = [[MASViewConstraint alloc]
      initWithFirstViewAttribute:self.firstViewAttribute];
...
- (void)setSecondViewAttribute:(id)secondViewAttribute {
  if ([secondViewAttribute isKindOfClass:NSValue.class]) {
    ...
    NSAssert(NO, @"attempting to add unsupported attribute: %@",
             secondViewAttribute);
  }
}
...
- (void)setInset:(CGFloat)inset {
  [self setInsets:(MASEdgeInsets){.top = inset,
                                  .left = inset,
                                  .bottom = inset,
                                  .right = inset}];
}
...
- (void)install {
  ...
  NSLayoutAttribute firstLayoutAttribute =
      self.firstViewAttribute.layoutAttribute;
  MAS_VIEW *secondLayoutItem = self.secondViewAttribute.item;
  NSLayoutAttribute secondLayoutAttribute =
      self.secondViewAttribute.layoutAttribute;
...
- (MASLayoutConstraint *)layoutConstraintSimilarTo:
    (MASLayoutConstraint *)layoutConstraint {
  // check if any constraints are the same apart from the only mutable property
  // constant

  // go through constraints in reverse as we do not want to match auto-resizing
  // or interface builder constraints and they are likely to be added first.
  for (NSLayoutConstraint *existingConstraint in self.installedView.constraints
