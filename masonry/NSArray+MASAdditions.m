      ...
      [v mas_makeConstraints:^(MASConstraintMaker *make) {
        ...
        if (prev) {
          if (i == self.count - 1) { // last one
            ...
            CGFloat offset = (1 - (i / ((CGFloat)self.count - 1))) *
                                 (fixedItemLength + leadSpacing) -
                             i * tailSpacing / (((CGFloat)self.count - 1));
            make.right.equalTo(tempSuperView)
                .multipliedBy(i / ((CGFloat)self.count - 1))
                .with.offset(offset);
          }
        } else { // first one
          make.left.equalTo(tempSuperView).offset(leadSpacing);
...
- (MAS_VIEW *)mas_commonSuperviewOfViews {
  ...
  NSAssert(commonSuperview,
           @"Can't constrain views that do not share a common superview. Make "
